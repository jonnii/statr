...
task PreparePackage -depends Test {
	...
	cp ..\targets\statr.client\statr.* ..\targets\packages\lib\net40
}
...
task Test -depends Clean,CopyTools,Compile { 
  # run specs
  ..\src\packages\Machine.Specifications.0.5.8\tools\mspec-clr4.exe .\..\targets\statr.client.specifications\Statr.Client.Specifications.dll
...
task Compile -depends Clean,UpdateAssemblyInfo { 
  ...
  Push-Location ..\src
  $build = $msbuild + ' "Statr.sln" ' + $options + " /t:Build"

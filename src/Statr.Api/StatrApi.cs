...
namespace Statr.Api
{
    public class StatrApi : IStatrApi
    {
        ...
        public IEnumerable<DataPoint> DataPoints(string metricType, string bucket, int limit = 200)
        {
            return Client.Get("datapoints/:metricType/:bucket", new { metricType, bucket, limit })
                .OnOk()
                .As<List<DataPoint>>();
        }
    }
